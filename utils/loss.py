...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.BCEmkp = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device))
    ...
    def __call__(self, p, targets, imgs, pre_mask):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj, lmkp = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        gt_mask = None
        if pre_mask:
            bs, as_, gjs, gis, targets, anchors, gt_mask = self.build_targets_mask(p, targets, imgs, pre_mask)
        else:
        ...
        # 二值mask引导的位置损失
        if pre_mask:
            for index, ipm in enumerate(pre_mask):
                lmkp += self.BCEmkp(pre_mask[index], gt_mask[index])
        # ------------------------------------------------------
        ...
        loss = lbox + lobj + lcls + lmkp
        return loss * bs, torch.cat((lbox, lobj, lcls, lmkp, loss)).detach()

    def build_targets_mask(self, p, targets, imgs, pre_mask):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            if pre_mask:
                ...
                this_gt_mask = []
                for pm in pre_mask:
                    maskxywh = this_target[:, 2:6] * pm.shape[2]
                    maskxyxy = xywh2xyxy(maskxywh)
                    this_gm = get_gt_mask(this_target, maskxyxy, pm)
                    # get_gt_mask(this_target, xywh2xyxy(this_target[:, 2:6] * imgs.shape[2]), imgs)
                    this_gt_mask.append(this_gm)
                batch_gt_mask.append(this_gt_mask)
                # -----------------------------------------
        ...
        if pre_mask:
            # -----------------------------------
            gt_mask = []
            for i, ipm in enumerate(pre_mask):
                gt_mask.append(torch.cat([gm[i] for gm in batch_gt_mask], dim=0))
            # -----------------------------------

        return matching_bs, matching_as, matching_gjs, matching_gis, matching_targets, matching_anchs, gt_mask
...
def get_gt_mask(targets, txyxy, pre_mask):
    xx1 = torch.zeros(pre_mask.shape[2], pre_mask.shape[3])
    yy1 = xx1.cpu().numpy()
    for xyxy in txyxy:
        yy1[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])] = 1

    yy = torch.tensor(yy1, device=targets.device).view(1, 1, yy1.shape[0], yy1.shape[1])
    # oo = pre_mask * yy
    # pp = imgs * (1 - yy)
    # plt.imshow(transforms.ToPILImage()(yy[0][0]), interpolation="bicubic")
    # transforms.ToPILImage()(yy[0][0]).show()  # Alternatively
    # plt.imshow(transforms.ToPILImage()(pre_mask[0][0]), interpolation="bicubic")
    # transforms.ToPILImage()(pre_mask[0][0]).show()  # Alternatively
    # plt.imshow(transforms.ToPILImage()(oo[0][0]), interpolation="bicubic")
    # transforms.ToPILImage()(oo[0][0]).show()  # Alternatively
    # plt.imshow(transforms.ToPILImage()(pp.squeeze(0)), interpolation="bicubic")
    # transforms.ToPILImage()(pp.squeeze(0)).show()  # Alternatively
    # ----------------------------------------------
    return yy
