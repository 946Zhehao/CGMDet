...
from utils.add_nms import LOGGER
...
##### 可视化特征图
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('H:/exp4'), imgname=None):
    """
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            f = save_dir / f"{imgname}_stage{stage}_{module_type.split('.')[-1]}_features.png"  # filename
            f = save_dir / f"stage{stage}_{imgname}.png"  # filename
            ...
            for i in range(n):
                ax[i].imshow(torch.tensor(blocks[i], dtype=torch.float32).squeeze())  # cmap='gray'
                ax[i].axis('off')
