...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, CnIoU=False, eps=1e-7):
    ...
    if GIoU or DIoU or CIoU:
        ...
        rlb = math.sqrt((b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2)
        rrt = math.sqrt((b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2)
        cx = (torch.max(b1_x2, b2_x2) + torch.min(b1_x1, b2_x1)) / 2
        cy = (torch.max(b1_y2, b2_y2) + torch.min(b1_y1, b2_y1)) / 2
        rlbc1 = math.sqrt((b2_x1 - cx) ** 2 + (b2_y1 - cy) ** 2)
        rlbc2 = math.sqrt((b1_x1 - cx) ** 2 + (b1_y1 - cy) ** 2)
        rrtc1 = math.sqrt((b2_x2 - cx) ** 2 + (b2_y2 - cy) ** 2)
        rrtc2 = math.sqrt((b1_x2 - cx) ** 2 + (b1_y2 - cy) ** 2)
        return iou - (rlb / (rlbc1 + rlbc2 + eps) + rrt / (rrtc1 + rrtc2 + eps))
    else:
        return iou  # IoU
